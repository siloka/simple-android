...
class PatientsScreenController @Inject constructor(
  ...
  private fun toggleVisibilityOfScanCardButton(events: Observable<UiEvent>): Observable<UiChange> {
    ...
    return events
        .ofType<ScreenCreated>()
        .withLatestFrom(isScanCardFeatureEnabledStream) { _, isScanCardFeatureEnabled -> isScanCardFeatureEnabled }
        .map { isScanCardFeatureEnabled ->
          { ui: Ui -> ui.showScanCardButton(isScanCardFeatureEnabled) }
        }
  }
