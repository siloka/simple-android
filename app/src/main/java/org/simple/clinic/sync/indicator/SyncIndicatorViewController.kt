...
class SyncIndicatorViewController @Inject constructor(
  ...
  private fun updateIndicatorView(events: Observable<UiEvent>): Observable<UiChange> {
    val syncResultStream = lastSyncGroupResult.asObservable().distinctUntilChanged()

    return Observables.combineLatest(
        events.ofType<SyncIndicatorViewCreated>(),
        syncResultStream.filterAndUnwrapJust()) { _, result ->
